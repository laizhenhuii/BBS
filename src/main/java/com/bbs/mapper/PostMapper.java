...
//    int addPost(Post post);
    @Insert("insert into post_table(posterID, posterName, postTitle, mainPost, postTime, likeNumber, pageView, postContent, homeTop, personalTop, postBoutique, postIntegral, moduleType, lastPost, imageAddress,parentID)" +
            "values(#{posterID}, #{posterName}, #{postTitle}, #{mainPost}, #{postTime}, #{likeNumber}, #{pageView}, #{postContent}, #{homeTop}, #{personalTop}, #{postBoutique}, #{postIntegral}, #{moduleType}, #{lastPost}, #{imageAddress},#{parentID})")
    //添加一条新帖(返回1表示成功，0表示失败，下面类似)
    int addPost(Post post);
...
//    List<Post> findPostByMainID(int postID);
    @Select("select * from post_table where mainPost = #{postID} and parentID=0")
    //根据主贴id查找评论帖
    List<Post> findPostByMainID(int postID);
...
//    int updatePost(Post post);
    @Update("update post_table set posterID = #{posterID}, posterName = #{posterName}, postTitle = #{postTitle}, mainPost = #{mainPost}, postTime = #{postTime}, " +
            "likeNumber = #{likeNumber}, pageView = #{pageView}, postContent = #{postContent}, homeTop = #{homeTop}, postBoutique = #{postBoutique}, " +
            "postIntegral = #{postIntegral}, moduleType = #{moduleType},  lastPost = #{lastPost}, imageAddress = #{imageAddress},parentID=#{parentID} where postID = #{postID}")
    //修改某条帖子信息
    //请用findByPostID找到该条帖子，
    //然后将想要修改的字段覆盖，再传入此函数
    int updatePost(Post post);

    @Select("select * from post_table where parentID = #{postID}")
        //根据父级评论id查找评论回复
    List<Post> findAnswerByParentID(int postID);
